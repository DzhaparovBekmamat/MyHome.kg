...
//Этот код представляет собой модуль Dagger Hilt для внедрения зависимостей в приложении Android
@Module
...
//    **fun provideDoorRepository(doorDao: DoorDao): DoorRepositoryImpl**:
//    - Аналогично предыдущему методу, этот метод предоставляет репозиторий `DoorRepositoryImpl`, который использует `DoorDao` для выполнения операций с данными двери.
